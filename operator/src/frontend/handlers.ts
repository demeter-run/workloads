...
import { CustomResource, CustomResourceResponse, Frontend, Pod } from '@demeter-run/workloads-types';
...
export async function handleResource(ns: string, name: string, spec: Frontend.Spec, owner: CustomResource<Frontend.Spec, Frontend.Status>): Promise<void> {
...
export async function updateResource(ns: string, name: string, spec: Frontend.Spec, containers: V1Container[], volumes: V1Volume[] | undefined, envVars: V1EnvVar[]): Promise<void> {
...
function buildSocatArgs(spec: Frontend.Spec) {
...
function deployment(name: string, spec: Frontend.Spec, owner: CustomResource<Frontend.Spec, Frontend.Status>, containers: V1Container[], volumes: V1Volume[] | undefined): V1Deployment {
    return {
        metadata: {
            name,
            labels: {
...
async function loadResource(ns: string, name: string) {
    ...
    const res = await crd.getNamespacedCustomObject(API_GROUP, API_VERSION, ns, PLURAL, name) as CustomResourceResponse<Frontend.Spec, Frontend.Status>;
    return res.body;
}